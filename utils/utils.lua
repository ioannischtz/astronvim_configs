...
function M.create_module_commands(module_name, commands)
  for command_name, def in pairs(commands) do
    ...
    api.nvim_create_user_command(
      command_name,
      function(info) require("lspconfig")[module_name].commands[command_name][1](unpack(info.fargs)) end,
      opts
    )
  end
end
...
local function check_in_workspace(client, root_dir)
  if not client.workspace_folders then return false end

  for _, dir in ipairs(client.workspace_folders) do
    if (root_dir .. "/"):sub(1, #dir.name + 1) == dir.name .. "/" then return true end
  end
...
function M.server_per_root_dir_manager(make_config)
  ...
  function manager.add(root_dir, single_file, bufnr)
    ...
    local function get_client_from_cache(client_name)
      if vim.tbl_count(clients) == 0 then return end

      if clients[root_dir] then
        for _, id in pairs(clients[root_dir]) do
          local client = lsp.get_client_by_id(id)
          if client and client.name == client_name then return client end
        end
      end

      local all_client_ids = {}
      vim.tbl_map(function(val) vim.list_extend(all_client_ids, { unpack(val) }) end, clients)
    ...
    local function attach_and_cache(root, client_id)
      ...
      if not clients[root] then clients[root] = {} end
      if not vim.tbl_contains(clients[root], client_id) then clients[root][#clients[root] + 1] = client_id end
    end
    ...
    local function start_new_client()
      ...
      if new_config.enabled == false then return end
      ...
      new_config.on_exit = M.add_hook_before(new_config.on_exit, function()
        for index, id in pairs(clients[root_dir]) do
          ...
          if exist.name == new_config.name then table.remove(clients[root_dir], index) end
        end
      end)

      -- Launch the server in the root directory used internally by lspconfig, if otherwise unset
      -- also check that the path exist
      if not new_config.cmd_cwd and uv.fs_realpath(root_dir) then new_config.cmd_cwd = root_dir end
    ...
    local function attach_or_spawn(client)
      if check_in_workspace(client, root_dir) then return attach_and_cache(root_dir, client.id) end

      local supported = vim.tbl_get(client, "server_capabilities", "workspace", "workspaceFolders", "supported")
      if supported then return register_workspace_folders(client) end
      start_new_client()
    end
    ...
    if not client then return start_new_client() end
  ...
  function manager.clients()
    ...
    for _, client_ids in pairs(clients) do
      for _, id in ipairs(client_ids) do
        local client = lsp.get_client_by_id(id)
        if client then res[#res + 1] = client end
      end
    end
    return res
  end
  ...
  local function matcher(path)
    for _, pattern in ipairs(patterns) do
      for _, p in ipairs(vim.fn.glob(M.path.join(M.path.escape_wildcards(path), pattern), true, true)) do
        if M.path.exists(p) then return path end
      end
    end
  end
...
function M.find_git_ancestor(startpath)
  return M.search_ancestors(startpath, function(path)
    ...
    if M.path.is_dir(M.path.join(path, ".git")) or M.path.is_file(M.path.join(path, ".git")) then return path end
  end)
end
...
function M.find_mercurial_ancestor(startpath)
  return M.search_ancestors(startpath, function(path)
    ...
    if M.path.is_dir(M.path.join(path, ".hg")) then return path end
  end)
end
...
function M.get_active_clients_list_by_ft(filetype)
  ...
  for _, client in pairs(clients) do
    ...
    for _, ft in pairs(filetypes) do
      if ft == filetype then table.insert(clients_list, client.name) end
    end
  end
  return clients_list
end
...
function M.get_active_client_by_name(bufnr, servername)
  for _, client in pairs(vim.lsp.get_active_clients { bufnr = bufnr }) do
    if client.name == servername then return client end
  end
end
...
function M.available_servers()
  ...
  for server, config in pairs(configs) do
    if config.manager ~= nil then table.insert(servers, server) end
  end
  return servers
end
